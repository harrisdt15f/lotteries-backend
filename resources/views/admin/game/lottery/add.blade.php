...
@section("content")
    <form id="addForm"  class="layui-form" action="{{route("lotteryAdd", [$model->id])}}">
        {{ csrf_field() }}
        <fieldset>
            @if($model->id > 0)
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="series_id" id="series_id" lay-verify="required" lay-filter="merchant_sign">
                        @foreach($series as $sign => $name)
                            <option value="{{$sign}}" @if ($model->id && $sign == $model->series_id) selected @endif>{{$name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
